...
router.get("/get", async (req, res) => {
  ...
  try {
    const searchRes = await shopifyRequest(`customers/search.json?query=email:${email}`, "GET");
    ...
    const metaRes = await shopifyRequest(`customers/${customerId}/metafields.json`, "GET");
    const metas = JSON.parse(metaRes.data).metafields;
    const wishlistMeta = metas.find(m => m.namespace === "custom" && m.key === "wishlist");
...
router.post("/remove", async (req, res) => {
  ...
  if (!email || (!productTitle && !variantId)) {
    return res.status(400).json({ error: "Missing email and product identifier" });
  }
  ...
  try {
    ...
    if (!wishlistMeta) return res.status(404).json({ error: "Wishlist not found" });

    let wishlist = JSON.parse(wishlistMeta.value);

    const filterFn = p =>
      variantId ? p.variantId !== variantId : p.productTitle.replace(/'/g, "") !== productTitle.replace(/'/g, "");
    ...
    const saveRes = await shopifyRequest(`customers/${customerId}/metafields/${wishlistMeta.id}.json`, "PUT", payload);
    if (!saveRes.ok) throw new Error("Failed to update wishlist");

    res.json({ success: true, message: "Item removed", wishlist: wishlist["all"] || [] });
...
router.post("/toggle", async (req, res) => {
  ...
  if (!email || !bucket || !newItem) return res.status(400).json({ error: "Missing required fields" });
  ...
  try {
    ...
    const exists = wishlist[bucket].some(p => p.variantId === newItem.variantId);

    if (exists) {
      wishlist[bucket] = wishlist[bucket].filter(p => p.variantId !== newItem.variantId);
      wishlist["all"] = wishlist["all"].filter(p => p.variantId !== newItem.variantId);
    } else {
...
router.post("/add", async (req, res) => {
  ...
  try {
    ...
    const alreadyExists = wishlist[bucket].some(p => p.variantId === newItem.variantId);
